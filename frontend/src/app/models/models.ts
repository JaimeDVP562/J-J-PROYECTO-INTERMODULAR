...
export interface Factura {
  ...
  invoice_type?: string;
  rectified_invoice?: string | null;
  total_amount: number;
  gross_amount?: number;
  tax_amount?: number;
  tax_breakdown?: Array<{ type: string; rate: number; base: number; quota: number }>;
  status: 'pending' | 'paid' | 'cancelled' | 'draft' | 'issued' | 'sent' | 'accepted' | 'rejected';
  invoice_date: string;
  due_date?: string;
